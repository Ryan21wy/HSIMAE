...
class FDSSC_f(nn.Module):
    def __init__(self, band, classes):
        ...
        # spectral branch
        print('FDSSC_f')
        self.conv1 = nn.Conv1d(in_channels=1, out_channels=24, kernel_size=7, stride=2)

        # Dense block
        self.batch_norm1 = nn.Sequential(
            nn.BatchNorm1d(24, eps=0.001, momentum=0.1, affine=True),  # 动量默认值为0.1
            nn.PReLU()
        )
        self.conv2 = nn.Conv1d(in_channels=24, out_channels=12, padding=3, kernel_size=7, stride=1)
        self.batch_norm2 = nn.Sequential(
            nn.BatchNorm1d(36, eps=0.001, momentum=0.1, affine=True),
            nn.PReLU()
        )

        self.conv3 = nn.Conv1d(in_channels=36, out_channels=12, padding=3, kernel_size=7, stride=1)
        self.batch_norm3 = nn.Sequential(
            nn.BatchNorm1d(48, eps=0.001, momentum=0.1, affine=True),
            nn.PReLU()
        )
        self.conv4 = nn.Conv1d(in_channels=48, out_channels=12, padding=3, kernel_size=7, stride=1)
        self.batch_norm4 = nn.Sequential(
            nn.BatchNorm1d(60, eps=0.001, momentum=0.1, affine=True),
            nn.PReLU()
        )

        kernel_3d = math.ceil((band - 6) / 2)
        self.conv5 = nn.Conv1d(in_channels=60, out_channels=200, kernel_size=kernel_3d, stride=1)
        self.batch_norm5 = nn.Sequential(
            nn.BatchNorm1d(200, eps=0.001, momentum=0.1, affine=True),
            nn.PReLU()
        )

        self.conv6 = nn.Conv2d(in_channels=200, out_channels=24, kernel_size=(3, 3), stride=(1, 1))
        self.batch_norm6 = nn.Sequential(
            nn.BatchNorm2d(24, eps=0.001, momentum=0.1, affine=True),  # 动量默认值为0.1
            nn.PReLU()
        )
        self.conv7 = nn.Conv2d(in_channels=24, out_channels=12, padding=(1, 1), kernel_size=(3, 3), stride=(1, 1))
        self.batch_norm7 = nn.Sequential(
            nn.BatchNorm2d(36, eps=0.001, momentum=0.1, affine=True),
            nn.PReLU()
        )
        self.conv8 = nn.Conv2d(in_channels=36, out_channels=12, padding=(1, 1), kernel_size=(3, 3), stride=(1, 1))
        self.batch_norm8 = nn.Sequential(
            nn.BatchNorm2d(48, eps=0.001, momentum=0.1, affine=True),
            nn.PReLU()
        )
        self.conv9 = nn.Conv2d(in_channels=48, out_channels=12, padding=(1, 1), kernel_size=(3, 3), stride=(1, 1))
        self.batch_norm9 = nn.Sequential(
    ...
    def forward(self, X):
        ...
        # spectral
        X = rearrange(X, 'n c h w b -> (n h w) c b')
        x1 = self.conv1(X)
        x2 = self.batch_norm1(x1)
        x2 = self.conv2(x2)
        ...
        # 光谱注意力通道
        x7 = rearrange(x7, '(n h w) c b -> n (c b) h w', h=h, w=w)
        x7 = self.conv6(x7)
